  ...
  <div class="pa-4 text-center">
    <v-dialog width="auto" scrollable>
      <template v-slot:activator="{ props: activatorProps }">
        <v-fab color="brown" prepend-icon="mdi-earth" absolute offset v-bind="activatorProps"></v-fab>
      </template>
      ...
      <template v-slot:default="{ isActive }">
        <v-card prepend-icon="mdi-earth" title="Select Country">
          ...
          <v-card-text class="px-4" style="height: 300px">
            <v-radio-group v-model="dialog" messages="Select a Country from the radio group" column>
          ...
          <v-card-actions>
            ...
            <v-btn color="surface-variant" text="Save" variant="flat" @click="isActive.value = false"></v-btn>
          </v-card-actions>
        </v-card>
      </template>
    </v-dialog>
