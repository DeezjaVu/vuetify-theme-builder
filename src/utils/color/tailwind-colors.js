/**
 * Tailwind Preset Colors
 *
 * @see https://tailwindcolor.com/
 *
 */

export const red = {
  lighten5: "#FEF2F2",
  lighten4: "#FEE2E2",
  lighten3: "#FECACA",
  lighten2: "#FCA5A5",
  lighten1: "#F87171",
  base: "#EF4444",
  darken1: "#DC2626",
  darken2: "#B91C1C",
  darken3: "#991B1B",
  darken4: "#7F1D1D"
};

export const rose = {
  lighten5: "#FFF1F2",
  lighten4: "#FFE4E6",
  lighten3: "#FECDD3",
  lighten2: "#FDA4AF",
  lighten1: "#FB7185",
  base: "#F43F5E",
  darken1: "#E11D48",
  darken2: "#BE123C",
  darken3: "#9F1239",
  darken4: "#881337"
};

export const pink = {
  lighten5: "#FDF2F8",
  lighten4: "#FCE7F3",
  lighten3: "#FBCFE8",
  lighten2: "#F9A8D4",
  lighten1: "#F472B6",
  base: "#EC4899",
  darken1: "#DB2777",
  darken2: "#BE185D",
  darken3: "#9D174D",
  darken4: "#831843"
};

export const fuchsia = {
  lighten5: "#FDF4FF",
  lighten4: "#FAE8FF",
  lighten3: "#F5D0FE",
  lighten2: "#F0ABFC",
  lighten1: "#E879F9",
  base: "#D946EF",
  darken1: "#C026D3",
  darken2: "#A21CAF",
  darken3: "#86198F",
  darken4: "#701A75"
};

export const purple = {
  lighten5: "#FAF5FF",
  lighten4: "#F3E8FF",
  lighten3: "#E9D5FF",
  lighten2: "#D8B4FE",
  lighten1: "#C084FC",
  base: "#A855F7",
  darken1: "#9333EA",
  darken2: "#7E22CE",
  darken3: "#6B21A8",
  darken4: "#581C87"
};

export const violet = {
  lighten5: "#F5F3FF",
  lighten4: "#EDE9FE",
  lighten3: "#DDD6FE",
  lighten2: "#C4B5FD",
  lighten1: "#A78BFA",
  base: "#8B5CF6",
  darken1: "#7C3AED",
  darken2: "#6D28D9",
  darken3: "#5B21B6",
  darken4: "#4C1D95"
};

export const indigo = {
  lighten5: "#EEF2FF",
  lighten4: "#E0E7FF",
  lighten3: "#C7D2FE",
  lighten2: "#A5B4FC",
  lighten1: "#818CF8",
  base: "#6366F1",
  darken1: "#4F46E5",
  darken2: "#4338CA",
  darken3: "#3730A3",
  darken4: "#312E81"
};

export const blue = {
  lighten5: "#EFF6FF",
  lighten4: "#DBEAFE",
  lighten3: "#BFDBFE",
  lighten2: "#93C5FD",
  lighten1: "#60A5FA",
  base: "#3B82F6",
  darken1: "#2563EB",
  darken2: "#1D4ED8",
  darken3: "#1E40AF",
  darken4: "#1E3A8A"
};

export const sky = {
  lighten5: "#F0F9FF",
  lighten4: "#E0F2FE",
  lighten3: "#BAE6FD",
  lighten2: "#7DD3FC",
  lighten1: "#38BDF8",
  base: "#0EA5E9",
  darken1: "#0284C7",
  darken2: "#0369A1",
  darken3: "#075985",
  darken4: "#0C4A6E"
};

export const cyan = {
  lighten5: "#ECFEFF",
  lighten4: "#CFFAFE",
  lighten3: "#A5F3FC",
  lighten2: "#67E8F9",
  lighten1: "#22D3EE",
  base: "#06B6D4",
  darken1: "#0891B2",
  darken2: "#0E7490",
  darken3: "#155E75",
  darken4: "#164E63"
};

export const teal = {
  lighten5: "#F0FDFA",
  lighten4: "#CCFBF1",
  lighten3: "#99F6E4",
  lighten2: "#5EEAD4",
  lighten1: "#2DD4BF",
  base: "#14B8A6",
  darken1: "#0D9488",
  darken2: "#0F766E",
  darken3: "#115E59",
  darken4: "#134E4A"
};

export const emerald = {
  lighten5: "#ECFDF5",
  lighten4: "#D1FAE5",
  lighten3: "#A7F3D0",
  lighten2: "#6EE7B7",
  lighten1: "#34D399",
  base: "#10B981",
  darken1: "#059669",
  darken2: "#047857",
  darken3: "#065F46",
  darken4: "#064E3B"
};

export const green = {
  lighten5: "#F0FDF4",
  lighten4: "#DCFCE7",
  lighten3: "#BBF7D0",
  lighten2: "#86EFAC",
  lighten1: "#4ADE80",
  base: "#22C55E",
  darken1: "#16A34A",
  darken2: "#15803D",
  darken3: "#166534",
  darken4: "#14532D"
};

export const lime = {
  lighten5: "#F7FEE7",
  lighten4: "#ECFCCB",
  lighten3: "#D9F99D",
  lighten2: "#BEF264",
  lighten1: "#A3E635",
  base: "#84CC16",
  darken1: "#65A30D",
  darken2: "#4D7C0F",
  darken3: "#3F6212",
  darken4: "#365314"
};

export const yellow = {
  lighten5: "#FEFCE8",
  lighten4: "#FEF9C3",
  lighten3: "#FEF08A",
  lighten2: "#FDE047",
  lighten1: "#FACC15",
  base: "#EAB308",
  darken1: "#CA8A04",
  darken2: "#A16207",
  darken3: "#854D0E",
  darken4: "#713F12"
};

export const amber = {
  lighten5: "#FFFBEB",
  lighten4: "#FEF3C7",
  lighten3: "#FDE68A",
  lighten2: "#FCD34D",
  lighten1: "#FBBF24",
  base: "#F59E0B",
  darken1: "#D97706",
  darken2: "#B45309",
  darken3: "#92400E",
  darken4: "#78350F"
};

export const orange = {
  lighten5: "#FFF7ED",
  lighten4: "#FFEDD5",
  lighten3: "#FED7AA",
  lighten2: "#FDBA74",
  lighten1: "#FB923C",
  base: "#F97316",
  darken1: "#EA580C",
  darken2: "#C2410C",
  darken3: "#9A3412",
  darken4: "#7C2D12"
};

export const zinc = {
  lighten5: "#FAFAFA",
  lighten4: "#F4F4F5",
  lighten3: "#E4E4E7",
  lighten2: "#D4D4D8",
  lighten1: "#A1A1AA",
  base: "#71717A",
  darken1: "#52525B",
  darken2: "#3F3F46",
  darken3: "#27272A",
  darken4: "#18181B"
};

export const stone = {
  lighten5: "#FAFAF9",
  lighten4: "#F5F5F4",
  lighten3: "#E7E5E4",
  lighten2: "#D6D3D1",
  lighten1: "#A8A29E",
  base: "#78716C",
  darken1: "#57534E",
  darken2: "#44403C",
  darken3: "#292524",
  darken4: "#1C1917"
};

export const neutral = {
  lighten5: "#FAFAFA",
  lighten4: "#F5F5F5",
  lighten3: "#E5E5E5",
  lighten2: "#D4D4D4",
  lighten1: "#A3A3A3",
  base: "#737373",
  darken1: "#525252",
  darken2: "#404040",
  darken3: "#262626",
  darken4: "#171717"
};

export const gray = {
  lighten5: "#F9FAFB",
  lighten4: "#F3F4F6",
  lighten3: "#E5E7EB",
  lighten2: "#D1D5DB",
  lighten1: "#9CA3AF",
  base: "#6B7280",
  darken1: "#4B5563",
  darken2: "#374151",
  darken3: "#1F2937",
  darken4: "#111827"
};

export const slate = {
  lighten5: "#F8FAFC",
  lighten4: "#F1F5F9",
  lighten3: "#E2E8F0",
  lighten2: "#CBD5E1",
  lighten1: "#94A3B8",
  base: "#64748B",
  darken1: "#475569",
  darken2: "#334155",
  darken3: "#1E293B",
  darken4: "#0F172A"
};

/**
 * Tailwind base color list.
 *
 * Each item in the list is an object with the following properties:
 * - title : name of the color in title case, usable for display.
 * - name: name of the color in camel case.
 * - base: base color of the color in hex.
 *
 * @returns {array} An array of objects.
 *
 * @see https://tailwindcss.com/docs/customizing-colors
 */
export const baseColors = [
  { title: "Red", name: "red", base: red.base },
  { title: "Rose", name: "rose", base: rose.base },
  { title: "Pink", name: "pink", base: pink.base },
  { title: "Fuchsia", name: "fuchsia", base: fuchsia.base },
  { title: "Purple", name: "purple", base: purple.base },
  { title: "Violet", name: "violet", base: violet.base },
  { title: "Indigo", name: "indigo", base: indigo.base },
  { title: "Blue", name: "blue", base: blue.base },
  { title: "Sky", name: "sky", base: sky.base },
  { title: "Cyan", name: "cyan", base: cyan.base },
  { title: "Teal", name: "teal", base: teal.base },
  { title: "Emerald", name: "emerald", base: emerald.base },
  { title: "Green", name: "green", base: green.base },
  { title: "Lime", name: "lime", base: lime.base },
  { title: "Yellow", name: "yellow", base: yellow.base },
  { title: "Amber", name: "amber", base: amber.base },
  { title: "Orange", name: "orange", base: orange.base },
  { title: "Slate", name: "slate", base: slate.base },
  { title: "Gray", name: "gray", base: gray.base },
  { title: "Zinc", name: "zinc", base: zinc.base },
  { title: "Neutral", name: "neutral", base: neutral.base },
  { title: "Stone", name: "stone", base: stone.base }
];

const redHex = Object.values(red).reverse();
const roseHex = Object.values(rose).reverse();
const pinkHex = Object.values(pink).reverse();
const fuchsiaHex = Object.values(fuchsia).reverse();
const purpleHex = Object.values(purple).reverse();
const violetHex = Object.values(violet).reverse();
const indigoHex = Object.values(indigo).reverse();
const blueHex = Object.values(blue).reverse();
const skyHex = Object.values(sky).reverse();
const cyanHex = Object.values(cyan).reverse();
const tealHex = Object.values(teal).reverse();
const emeraldHex = Object.values(emerald).reverse();
const greenHex = Object.values(green).reverse();
const limeHex = Object.values(lime).reverse();
const yellowHex = Object.values(yellow).reverse();
const amberHex = Object.values(amber).reverse();
const orangeHex = Object.values(orange).reverse();
const zincHex = Object.values(zinc).reverse();
const slateHex = Object.values(slate).reverse();
const grayHex = Object.values(gray).reverse();
const neutralHex = Object.values(neutral).reverse();
const stoneHex = Object.values(stone).reverse();

export const swatches = [
  redHex,
  roseHex,
  pinkHex,
  fuchsiaHex,
  purpleHex,
  violetHex,
  indigoHex,
  blueHex,
  skyHex,
  cyanHex,
  tealHex,
  emeraldHex,
  greenHex,
  limeHex,
  yellowHex,
  amberHex,
  orangeHex,
  zincHex
  // slateHex,
  // grayHex,
  // neutralHex,
  // stoneHex
];

// TODO: @tailwind-colors ::: fix theme colors.
export const theme = {
  primary: "#2196F3",
  secondary: "#8BC34A",
  success: "#2ECC71",
  info: "#2196F3",
  warning: "#FFC107",
  danger: "#FFC080"
};

export default {
  red,
  rose,
  pink,
  fuchsia,
  purple,
  violet,
  indigo,
  blue,
  sky,
  cyan,
  teal,
  emerald,
  green,
  lime,
  yellow,
  amber,
  orange,
  slate,
  gray,
  zinc,
  neutral,
  stone,
  baseColors,
  swatches,
  theme
};
