...
import { theme } from "@/utils/color/flat-colors";
  ...
  function updateThemeStyleForItem(themeColor, includeOnColors) {
    ...
    if (name === "surfaceVariant") {
      ...
      if (includeOnColors) {
        ...
        // Delete the `on` color for the themeColor so that Vuetify generates default on-colors.
        delete themeDef.colors[onName];
        // When on-colors is disabled, could potentially still add them as separate style.
...
export const useMaterialThemeStore = defineStore(
  ...
  () => {
    ...
    function updateThemeStyles() {
      ...
      console.warn(
        "MaterialThemeStore ::: updateThemeStyles &mdash; Calling this method will update all hct theme styles. Look into only updating the modified item styles if possible."
      );
    ...
    function updateThemeForPalette(palette) {
      ...
      let onColors = includeOnColors.value;
      console.log(" - includeOnColors: ", onColors);
      ...
      if (palette.name === "neutral") {
        ...
        materialStore.value.updateThemeStyleForItem(l1, onColors);
        materialStore.value.updateThemeStyleForItem(l2, onColors);
        materialStore.value.updateThemeStyleForItem(d1, onColors);
        materialStore.value.updateThemeStyleForItem(d2, onColors);
        ...
        materialStore.value.updateThemeStyleForItem(lightColor, onColors);
        materialStore.value.updateThemeStyleForItem(darkColor, onColors);
      } else {
